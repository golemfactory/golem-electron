    ...
    var path = Object.keys(data).map(function(anchestorKey) {
      return {
        ...
        value: Object.keys(data[anchestorKey]).map(function(parentKey) {
          return Object.keys(data[anchestorKey][parentKey]).map(function(
            childKey,
            index
          ) {
      ...
      .map((item, index) => {
        ...
        return !!subtask ? (
          <Tooltip
            ...
            content={
              <div
                className={`tooltip-frame ${isDevMode ? 'tooltip-dev' : ''}`}>
                <div className="content__tooltip">
                    ...
                    <div className="dev-tag__tooltip">
                      ...
                      <p
                        className={`time__tooltip ${
                          subtask.status === statusDict.FINISHED
                            ? 'time__tooltip--done'
                            : ''
                        }`}>
                        {timeStampToHR(subtask.time_started)}
                      </p>
                      {isDevMode && (
                      ...
                      {isDevMode && (
                        <p
                          ...
                          style={{
                            color: this.state.subtaskIdCopied[
                              subtask.subtask_id
                            ]
                              ? '#37c481'
                              : '#9b9b9b'
                          ...
                          <b>
                            {this.state.subtaskIdCopied[subtask.subtask_id]
                              ? 'Subtask ID copied!'
                  ...
                  <div className="resubmit_block_btns__tooltip">
                    ...
                    {isDevMode && (
                      <button
                        type="button"
                        onClick={this._showBlockNodeModal.bind(this, subtask)}>
                        Block node
                      </button>
                    )}
                  </div>
