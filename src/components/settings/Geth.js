...
export class Geth extends React.Component {
    ...
    _handleGethPort = e => this.setState({ gethPort: e.target.value || "" });

    _handleGethAddress = e => {
        e.target.checkValidity();
        if (!e.target.validity.valid) e.target.classList.add("invalid");
        else {
    ...
    _handleSave = e => {
        ...
        if (gethAddress || isLocalGeth) {
            //TODO check geth
            const deferred = mainProcess.validateGeth(
                isLocalGeth,
                gethAddress,
                gethPort,
                isMainnet
    ...
    render() {
        ...
        return (
            <form
                className="content__geth"
                onSubmit={!loadingGethCheck ? this._handleSave : undefined}>
                <div className="section__flag">
                    ...
                    <div className="switch-box switch-box--green">
                        <label className="switch">
                            <input
                                type="checkbox"
                                onChange={this._handleGethSwitch}
                                defaultChecked={isLocalGeth}
                    ...
                    <span>
                        Local Geth Port <br />
                        <span className="label_optional">(optional)</span>
                    </span>
                    <input
                        type="number"
                        min="0"
                        max="65535"
                        step={1}
                        defaultValue={gethPort}
                        onChange={this._handleGethPort}
                        aria-label="Port of Local Geth"
                ...
                <button
                    ...
                    {!loadingGethCheck ? "Save" : "Checking..."}
                </button>
                <div className="section__tips">
                    <span className="tips__geth">
                        Enabling custom geth option will require restart of the
                        application
                    </span>
                    {gethError && (
                        <span className="error_geth">{gethError}</span>
                    )}
                </div>
            </form>
        );
