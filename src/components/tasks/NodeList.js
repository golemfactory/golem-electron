...
class NodeList extends Component {
    ...
    _countStatus(data){
        ...
        if(result)
            return result[0].map((item, index) => <span key={index.toString()} className="indicator__node-count"><span className={`icon-status-dot ${statusDot(item)}`}/>{result[1][index]}</span>)
        return []
    }

    _fillNodeInfo(data){
        const {isDataCopied} = this.state
        return data.map(({subtask_id, status, node_name, node_ip_address, node_id}, index) => <tr key={index.toString()}>
                <td>
                <Tooltip
                ...
                <td>
                    <button type="button" className="btn btn--warning btn--small" onClick={this._showBlockNodeModal.bind(this, {node_name, node_id})} disabled={!node_id}>Block</button>
                </td>
            </tr>)
    }
